...
* Function list: initScanner(), processToken(), getNextState(), nextTokenClass(), aStateFuncAVID(), aStateFuncSVID(), aStateFuncIL(), aStateFuncFPL(), aStateFuncSL(), aStateFuncErr(), isKeyword()
*/
...
Token processToken(void) {
	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		case '_':  //redo this with temp buffer 
			...
			//_AND_
			if (bufferGetChar(sourceBuffer) == 'A' &&
				bufferGetChar(sourceBuffer) == 'N' &&
				bufferGetChar(sourceBuffer) == 'D' &&
			...
			if (bufferGetChar(sourceBuffer) == 'N' &&
				bufferGetChar(sourceBuffer) == 'O' &&
				bufferGetChar(sourceBuffer) == 'T' &&
				bufferGetChar(sourceBuffer) == '_') {
				currentToken.code = LOG_OP_T;
			...
			//_NOT_
			if (bufferGetChar(sourceBuffer) == 'O' &&
				bufferGetChar(sourceBuffer) == 'R' &&
				bufferGetChar(sourceBuffer) == '_') {
				currentToken.code = LOG_OP_T;
		...
		default: //else break and go to the other cases below
			...
			if (stateType[state] == ASWR) { //if it is retractable, retract it // add this back maybe ?  
				bufferRetract(sourceBuffer);
			}

			lexEnd = bufferGetCPosition(sourceBuffer); //set the correct end postion 
			lexLength = (int)lexEnd - (int)lexStart; //calculate the lexLength
			//TODO check lexLenght is valid (shrtmin shrtmax)

			lexemeBuffer = bufferCreate((short)lexLength + 2, 0, 'f'); //create a temporary buffer of the correct size
			...
			for (i = 0; i < lexLength; i++) 	//copy char in source buffer to lexeme buffer
			{
			...
			currentToken = finalStateTable[state](bufferGetString(lexemeBuffer, 0)); //call the correct function to parse the token 
...
int nextTokenClass(char c) {
	...
	*/
	...
	else if (c == CHARSEOF0 || c == CHARSEOF255 || c == EOF) {
		val = 6; //SEOF(6)
	}
	else {
		val = 7; //other(7)
...
Token aStateFuncAVID(char lexeme[]) {
	...
	if (kwindex != RT_FAIL_2) {
		currentToken.code = KW_T;
		currentToken.attribute.get_int = kwindex;
	}
	else {
		currentToken.code = AVID_T;
		strncpy(currentToken.attribute.vid_lex, lexeme, VID_LEN); //set the correct values cont'd
...
Token aStateFuncSVID(char lexeme[]) {
	...
	strncpy(currentToken.attribute.vid_lex, lexeme, VID_LEN - 1); //set the correct values cont'd
...
Token aStateFuncErr(char lexeme[]) {
	...
	if (len > ERR_LEN) {
		...
		currentToken.attribute.err_lex[ERR_LEN - 3] = CHARSEOF0;
		strcat(currentToken.attribute.err_lex, "...");
	}
	else {
		strcpy(currentToken.attribute.err_lex, lexeme);
